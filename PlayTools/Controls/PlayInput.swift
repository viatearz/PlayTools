...
@objc class PlayInput: NSObject {
    ...
    private func disableBuiltinMouse() {
    ...
    private func disableBuiltinKeyboard() {
        NotificationCenter.default.addObserver(
            ...
            queue: .main) { nofitication in
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    NotificationCenter.default.post(name: NSNotification.Name.GCKeyboardDidDisconnect, object: keyboard)
    ...
    private var unityViewIntialized = false

    @objc func shouldDisableUnityKeyCommands(_ view: UIView) -> Bool {
        if !self.unityViewIntialized {
            self.unityViewIntialized = true
    ...
    func sendKeyEventToUnity(key: String, pressed: Bool) -> Bool {
        ...
        // The following code is tightly tied to UnityView+Keyboard.mm.
